...
ユーザーが手で補完した **rank_type / 日本語名 / 英語名** を`rank-override.json` に保存して、次回以降の変換に自動反映します。
...
```json
{
  ...
  "items": [
    {
      "pmoe_id": "the_stringless",   # Paimon.moe データから抽出した 内部 ID
      "name_en": "The Stringless",   # pmoe_id から推測した英名
      "name_jp": "",                 # 和名 (おそらくほとんどが空欄)
      "gacha_type": "301"            # 原神内部の祈願種類判別 ID
    }
  ]
}
```
  ...
  "items": [
    {
      "pmoe_id": "magic_guide",
      "name_en": "Magic Guide",
      "name_jp": "魔導緒論",     # "和名" 手動補完ください。
      "rank_type": "3"          # "レア度（☆の数）" 手動補完ください。
    },
    {
      "pmoe_id": "rust",
      "name_en": "Rust",
